...
contains
    ...
    subroutine find_single_eigenvalue(d, e, N, eigenvalue_num, eigenvalue_out)
        ...
        real*8 :: low, high, mid, f_val
        integer :: i, count, max_iter
        
        ! Better initial bounds estimation
        low = minval(d) - 2.0d0 * maxval(abs(e))
        high = maxval(d) + 2.0d0 * maxval(abs(e))
        ...
        do i = 1, max_iter
            ...
            do concurrent (i = 2:N)  ! Use concurrent loop for better optimization
                if (abs(f_val) < EPSILON) then
                    f_val = EPSILON * sign(1.0d0, f_val)
                end if
                f_val = (d(i) - mid) - (e(i-1)**2 / f_val)
    ...
    ! Detailed analysis with two-stage fitting - optimized version
    subroutine detailed_eigenvalue_analysis(eigenvalues_full, N_full, known_zeta_zeros, &
        ...
        real*8, dimension(N_full) :: eigenvalues_local
        ...
        eigenvalues_local = eigenvalues_full
        call parallel_quicksort(eigenvalues_local, 1, N_full)
        
        call select_initial_eigenvalues(eigenvalues_local, fitted_eigenvalues, &
                                        N_full, n_known_zeros)
        ...
        ! Apply the fitting models to the fitted data - vectorized
        ...
        call select_next_eigenvalues(eigenvalues_local, predicted_eigenvalues, &
                                     N_full, n_known_zeros + 1, n_predict_zeros)
    ...
    ! Parallel quicksort implementation for better performance
    recursive subroutine parallel_quicksort(arr, low, high)
        implicit none
        integer, intent(in) :: low, high
        real*8, intent(inout) :: arr(:)
        integer :: pivot_idx, threshold
        
        threshold = 10000  ! Use parallel sort only for large arrays
